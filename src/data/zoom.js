...
export function getZoomCandidates(entities) {
    return entities.filter((entity) =>
        PICTURES.some(
            (picture) => picture.entityId === entity.id && picture.variations.length > 0
        )
    );
}

export function createRandomZoomChallenge(entities, excludedIds = []) {
    const availableCandidates = getZoomCandidates(entities).filter(
...
export function isZoomChallengeValid(challenge, entities) {
    if (!challenge?.entityId || !challenge?.darkSrc || !challenge?.clearSrc) {
        return false;
    }

    return getZoomCandidates(entities).some((entity) => entity.id === challenge.entityId);
}
